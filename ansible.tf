...
resource "null_resource" "foo" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "cd ~ ; git clone ${var.ansible.downloadGoogleDriveObjectUrl} --branch ${var.ansible.downloadGoogleDriveObjectTag} ; cd ${split("/", var.ansible.downloadGoogleDriveObjectUrl)[4]} ; ansible-playbook local.yml --extra-vars 'googleDriveId=${var.avi_googleId_20_1_4_qcow2}' --extra-vars 'outputFile=${var.openstack.glance[0].fileName}'",
  ...
  provisioner "file" {
    content      = <<EOF
{"openstack": ${jsonencode(var.openstack)}, "avi_controller": ${jsonencode(var.avi_controller)}, "extDefaultGw": ${jsonencode(split("/", var.jump.ipCidrData)[0])}, "openstackAviCredsFile": ${jsonencode(var.kolla.admin_avi)}}
EOF
